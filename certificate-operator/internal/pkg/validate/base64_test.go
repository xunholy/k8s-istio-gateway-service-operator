...
func TestCheckBase64Success(t *testing.T) {
	valid := validate.CheckBase64(cert)
	if !valid {
		t.Fatalf("expected decoding to be valid")
	}
	valid = validate.CheckBase64(key)
	if !valid {
		t.Fatalf("expected decoding to be valid")
	}
}

func TestCheckBase64Failure(t *testing.T) {
	// Removed first char of string
	_, i := utf8.DecodeRuneInString(cert)
	invalidCert := cert[i:]
	valid := validate.CheckBase64(invalidCert)
	if valid {
		t.Fatalf("expected decoding to be valid")
	}
}
